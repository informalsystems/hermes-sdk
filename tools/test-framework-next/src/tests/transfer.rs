...
use ibc_relayer_components::build::traits::birelay::HasBiRelay;
...
use crate::traits::chain::fields::token::{CanGenerateRandomTokenAmount, HasTokenFields};
...
where
    ...
    ChainA: HasIbcChainTypes<ChainB>
        ...
        + HasUserWallet<1>
        + HasDenom<0>,
    ChainB: HasIbcChainTypes<ChainA> + HasAddressType + HasWalletFields + HasUserWallet<0>,
{
    async fn run(test: &Test) -> Result<(), Test::Error> {
        ...
        let balance_a1 = chain_a.query_balance(address_a1, denom_a);

        let a_to_b_amount = ChainA::random_token_amount(denom_a, 1000, 5000);

        let channel_id_a = test.channel_id_a();

        let port_id_a = test.port_id_a();

        // chain_a.ibc_transfer_token(
        //     channel_id_a,
        //     port_id_a,
        //     wallet_a1,
        //     address_b,
