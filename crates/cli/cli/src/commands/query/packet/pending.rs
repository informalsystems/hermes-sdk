...
impl QueryPendingPackets {
    async fn execute(&self, builder: &CosmosBuilder) -> Result<Summary<PendingPackets>> {
                ...
                let chan_conn_cli = channel_connection_client(&handle, &port_id, &channel_id)
                    .map_err(|e| {
                        CosmosChain::raise_error(
                            e.1.wrap_err("failed to get channel connection and client"),
                        )
                    })?;
                Ok(chan_conn_cli)
            })
        ...
        let counterparty_chain = builder
            .build_chain(&counterparty_chain_id.clone())
            .await
            .wrap_err_with(|| format!("failed to build chain `{}`", counterparty_chain_id))?;
        ...
        .map_err(|e| {
            CosmosChain::raise_error(e.1.wrap_err("failed to get pending packet summary"))
        })?;
        ...
        .map_err(|e| {
            CosmosChain::raise_error(e.1.wrap_err("failed to get channel on destination"))
        })?
        .ok_or_else(|| {
