...
impl Runnable for QueryChannelEnd {
    async fn run(&self, builder: CosmosBuilder) -> Result<Output> {
        ...
        let channel_end = chain.with_blocking_chain_handle(move |chain_handle| {
            let query_height = if let Some(height) = height {
                let specified_height = Height::new(chain_handle.id().version(), height)
                .map_err(|e| BaseError::generic(eyre!("Failed to create Height with revision number `{}` and revision height `{height}`. Error: {e}", chain_handle.id().version())))?;
