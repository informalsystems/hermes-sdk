...
pub fn init_bootstrap_legacy(
    ...
    let cosmos_builder = Arc::new(CosmosBuilder::new_with_default(
        runtime.clone(),
        dynamic_gas.clone(),
    ));

    let chain_command_path = env::var("LEGACY_GAIA_BIN").unwrap_or_else(|_| "gaiad".to_string());
